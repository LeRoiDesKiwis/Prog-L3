...
static int my_semget(int nbreTokens)
{
    ...
    // explication (avec nbreTokens = 4)
    // Lorsque plusieurs workers vont vouloir entrer en SC, seuls les 4
    // premiers vont pouvoir faire -1. Le 5me sera bloqué sur un sémaphore à 0.
    // Dès qu'un worker sort de la SC, il fait +1 et libère une place pour un
    // worker en attente.
